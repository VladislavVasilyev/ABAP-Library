method get_filter.
  ...
  if i_memberselection is not initial.
    ...
    split i_memberselection at ds_badi_param-splitter into table lt_tab.

    loop at lt_tab assigning <ls_tab>.
      " RUN LOGIC FOR STRING
      split <ls_tab> at ds_badi_param-equal into l_dimstr l_memstr.
      lo_manager_ms->get_member_list_str( exporting i_selection = l_memstr
                                          importing e_selection = l_new_selection ).
      concatenate l_dimstr ds_badi_param-equal l_new_selection into ls_tabnew.
*      ls_tabnew = l_new_selection.
      append ls_tabnew to lt_tabnew.
    endloop.

    concatenate lines of lt_tabnew into e_filter_str separated by ds_badi_param-splitter.
  endif.
