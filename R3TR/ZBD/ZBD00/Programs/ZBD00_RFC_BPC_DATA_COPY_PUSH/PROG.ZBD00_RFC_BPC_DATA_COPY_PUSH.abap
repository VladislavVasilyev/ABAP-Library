...
REPORT  ZBD00_RFC_BPC_DATA_COPY_PUSH.
...
IF 'SELECTION SCREEN' = 'SELECTION SCREEN' .
  ...
  PARAMETERS: P_LOGSYS TYPE ZIDSYSTEM  OBLIGATORY VISIBLE LENGTH 18 . " matchcode object zbd_logsys
...
* checking for production and self system
    CONCATENATE `*` SY-SYSID `*` INTO LV_SYSPART.
    IF P_LOGSYS IS INITIAL.
      MESSAGE E001(00) WITH `Error! ` ` Target system is not set !` .
    ELSEIF P_LOGSYS CP '*BP2*'.
      MESSAGE E001(00) WITH `Error! ` ` Can not record data in the production system!` .
    ELSEIF P_LOGSYS CP LV_SYSPART.
*    if p_logsys cp lv_syspart.
      MESSAGE E001(00) WITH `Error!` ` Download data ONLY to other systems!` .
    ENDIF.

* destination rfc-address
    WRITE: / `Target system =` , P_LOGSYS .
    CASE SY-SYSID.
      WHEN 'BP0' .
        IF P_LOGSYS = 'BP1' .
          LV_DEST = 'BP1CLNT400_FOR_EXP'.
        ELSEIF P_LOGSYS = 'BP2' .
*       lv_dest = 'BP2CLNT500_FOR_EXP'.
        ENDIF.
      WHEN 'BP1' .
        IF P_LOGSYS = 'BP0' .
          LV_DEST = 'BP1_BP0CLNT100'.
        ELSEIF P_LOGSYS = 'BP2' .
*      lv_dest = 'BP2CLNT500_FOR_EXP'.
        ENDIF.
      WHEN 'BP2' .
        IF P_LOGSYS = 'BP0' .
          LV_DEST = 'BP0CLNT100'.
        ELSEIF P_LOGSYS = 'BP1' .
          LV_DEST = 'BP2_BP1CLNT400'.
        ENDIF.
    ENDCASE.

    IF LV_DEST IS INITIAL.
      MESSAGE E001(00) WITH `Error!` `RFC-connection is not defined !` .
    ELSE.
      LS_DEST = LV_DEST.
    ENDIF.
...
* clear target scoup
      IF 'CLEAR' = 'CLEAR'.
        ...
        IF LINES( LT_CLEAR_SEL ) > 0.
          ...
          CALL FUNCTION 'ZBD00_RFC_BPC_CLEAR' DESTINATION LV_DEST
            EXPORTING
              I_APPSET_ID         = P_APPSET
              I_APPL_ID           = LV_APPL_ID
...
*              if_suppress_zero    = abap_false
            ...
            EXCEPTIONS
              ERR_BD00_CREATE_OBJ = 1
              OTHERS              = 2.
          IF SY-SUBRC <> 0.
            CASE SY-SUBRC.
              WHEN 1 .
                CLEAR: LV_MSG_01, LV_MSG_02.
                CONCATENATE `Error!` ` Error occurred in the FM ` INTO LV_MSG_01 .
                CONCATENATE `ZBD00_RFC_BPC_CLEAR: ` `ERR_BD00_CREATE_OBJ.` INTO LV_MSG_02 .
                MESSAGE E001(00) WITH LV_MSG_01 LV_MSG_02 .
              WHEN OTHERS.
                CLEAR: LV_MSG_01, LV_MSG_02.
                CONCATENATE `Error!` ` Error occurred in the FM ` INTO LV_MSG_01 .
                CONCATENATE `ZBD00_RFC_BPC_CLEAR` `.` INTO LV_MSG_02 .
                MESSAGE E001(00) WITH LV_MSG_01 LV_MSG_02 .
            ENDCASE.
          ENDIF.
