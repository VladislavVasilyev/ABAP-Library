function ZBD00_RFC_BPC_READ_DEST .
*"--------------------------------------------------------------------
*"*"Локальный интерфейс:
*"  IMPORTING
...
*"     VALUE(I_AUTHORITY_CHECK) TYPE  RSDRI_AUTHCHK
...
*"      E_T_OUTDATA250 TYPE  BAPI6116TDA OPTIONAL
*"  CHANGING
...
*  l_first_call     = rs_c_true.
  ...
  if i_s_rfcmode is initial.
    l_packagesize = -1.
  else.
    l_packagesize = i_s_rfcmode-packagesize.
  endif.

*--------------------------------------------------------------------*
* Read data
*--------------------------------------------------------------------*
  while e_end_of_data = rs_c_false and lines( <lt_data> ) = 0.
  ...
  case ld_v__table_kind.
    when cl_abap_tabledescr=>tablekind_std or
      ...
      e_sup_rec = lines( <lt_data_hashed> ).
          <lt_data>[] = <lt_data_hashed>[].

      call function  'RSDRI_DATA_WRAP' " 'ZBD00_DATA_WRAP'
        exporting
          i_t_data         = <lt_data>
