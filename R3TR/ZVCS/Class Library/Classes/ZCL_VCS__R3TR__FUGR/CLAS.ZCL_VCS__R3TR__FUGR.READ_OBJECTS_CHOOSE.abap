method READ_OBJECTS_CHOOSE.
  ...
  data
  : ld_tr__fugr       type range of tadir-obj_name
  ...
  , <ld_s__rclas>      like line of ld_tr__fugr
  ...
  loop at cd_t__object assigning <ld_s__object>
    ...
    case <ld_s__object>-type.
      when `DEVC`.
        ...
        endloop.
      when `FUGR`.
        ld_tr__fugr = <ld_s__object>-obj_range.
    endcase.

  endloop.

  check ld_tr__fugr is not initial or ld_tr__devc is not initial.

  select *
    from  tadir
    into  corresponding fields of table ld_t__tadir
    where pgmid    = zvcsc_r3tr
      and object   = zvcsc_r3tr_type-fugr
      and obj_name in ld_tr__fugr
