method READ_OBJECTS_CHOOSE.
  ...
  data
  : ld_tr__view        type range of tadir-obj_name
  ...
  , <ld_s__rclas>      like line of ld_tr__view
  ...
  loop at cd_t__object assigning <ld_s__object>
    ...
    case <ld_s__object>-type.
      when zvcsc_r3tr_type-devc.
        ...
        endloop.
      when zvcsc_r3tr_type-view.
        ld_tr__view = <ld_s__object>-obj_range.
    endcase.

  endloop.

  check ld_tr__view is not initial or ld_tr__devc is not initial.

  select *
    from  tadir
    into  corresponding fields of table ld_t__tadir
    where pgmid    = zvcsc_r3tr
      and object   = zvcsc_r3tr_type-view
      and obj_name in ld_tr__view
